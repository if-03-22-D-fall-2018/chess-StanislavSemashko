...
ChessSquare* get_square(ChessBoard board, int file, int rank)
{
    ...
    {
        return &board[rank -1][file -97];
    }
    return 0;
}
bool is_square_occupied(ChessBoard board, int file, int rank)
{
    return board[rank -1][file -97].is_occupied;
}
bool add_piece(ChessBoard board, int file, int rank, struct ChessPiece piece){

    if(rank >=1 && rank <= 8 && file >= 'a' && file <='h' && board[rank -1][file -97].is_occupied)
    {
        board[rank -1][file -97].piece = piece;
        board[rank -1][file -97].is_occupied = true;
        return true;
    }
    return false;
}
...
{
   return board[rank -1][file -97].piece;
}
void setup_chess_board(ChessBoard board)
{
 ...
 add_piece(board,'a',7,white_rook);
 add_piece(board,'b',7,white_knight);
 add_piece(board,'c',7,white_bishop);
 add_piece(board,'d',7,white_queen);
 add_piece(board,'e',7,white_king);
 add_piece(board,'f',7,white_bishop);
 add_piece(board,'g',7,white_knight);
 add_piece(board,'h',7,white_rook);
}
...
{
    if(rank >=1 && rank <= 8 && file >= 'a' && file <='h' && ((file2 - 97)- (file -97)) == (rank2 - rank))
    {
        return true;
    }
    return false;
